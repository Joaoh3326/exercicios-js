(function() {
    ...
    */
    ...
    function hasClass( markup, cssClass ) {
        var regx = new RegExp( 'class=[\'"](?:[\\w\\s]+)?' + cssClass + '(?:[\\w\\s]+)?["\']');
        return regx.test( markup ) + ' para a classe ' + cssClass;
    }

    var array = [ 'container', 'text', 'date', 'excerpt', 'main' ];
    array.forEach(function( cssClass ) {
        return console.log( hasClass( markup, cssClass ) );
    });
